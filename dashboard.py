...
EMAIL_SENDER = "clustiv@gmail.com"
EMAIL_RECEIVER = "clustiv@gmail.com"
EMAIL_APP_PASSWORD = "Hanuman@1980"
...
# TITLE
# ----------------------------
st.title("🚀 AI Data Center Control Tower (CAIO)")

# ----------------------------
# KPIs
# ----------------------------
col1, col2, col3, col4 = st.columns(4)
col1.metric("⚡ Energy (kWh)", f"{energy_pred:.2f}")
col2.metric("💰 Cost (€)", f"{cost:.2f}")
col3.metric("⚠️ Risk", risk_level)
col4.metric("🛠 Failure Risk", failure_risk)
...
if role == "Executive":
    ...
    - Recommendation: Optimize cooling + load balancing
    """)
...
fig1 = px.line(combined, x='Timestamp', y='Energy_kWh',
               title="Energy Trend + Forecast")
st.plotly_chart(fig1, use_container_width=True)

# ----------------------------
# ANOMALY CHART
# ----------------------------
fig2 = px.scatter(df, x='Timestamp', y='Energy_kWh',
...
c1.plotly_chart(px.histogram(df, x='Server_Load_%', title="Server Load"))
c2.plotly_chart(px.histogram(df, x='Ambient_Temperature_C', title="Temperature"))
c3.plotly_chart(px.histogram(df, x='Cooling_Efficiency_%', title="Cooling Efficiency"))
...
if st.button("📧 Send Test Alert"):
    trigger_alert("Test Alert", "System working correctly")
